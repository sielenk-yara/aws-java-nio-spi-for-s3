 ...
 */
abstract class S3FileSystemProviderBase extends S3FileSystemProvider {

    private static final Map<String, S3FileSystemImpl> FS_CACHE = new ConcurrentHashMap<>();

    /**
     * This variable holds the configuration for the S3 NIO Service Provider Interface (SPI). It is used to manage and handle the configuration details required for interaction with S3 NIO services.
     *
     * @deprecated This variable is deprecated and may be removed in future versions. Consider using updated configuration mechanisms if available.
     */
    @Deprecated
     ...
     */
    protected Map<String, S3FileSystemImpl> getFsCache() {
     ...
     * @param env The environment to be used when creating the file system. May be null or empty. The following keys are supported:
     * <ul>
     * <li>acl</li>
     ...
     * @throws IOException If an exception occurs. In all cases the exception will wrap a causal exception which could be an SDKException thrown by the underlying S3 service or may be one of: ExecutionException, InterruptedException, or TimeoutException if a problem occurs with the asynchronous call to the service.
    ...
    public S3FileSystem newFileSystem(final URI uri, final Map<String, ?> env) throws IOException {
        ...
        } else if (!uri.getScheme().equals(getScheme())) {
        ...
        var config = new S3NioSpiConfiguration(envMap).withEndpoint(info.endpoint()).withBucketName(info.bucket());
        ...
        try (var client = new S3ClientProvider(config).configureCrtClient().build()) {
            var createBucketResponse = client.createBucket(bucketBuilder ->
                    bucketBuilder
                            .bucket(bucketName)
     ...
     */
    @Override
    public S3FileSystemImpl getFileSystem(URI uri) {
        var info = fileSystemInfo(uri);
        return FS_CACHE.computeIfAbsent(info.key(), (key) -> {
            ...
            return new S3FileSystemImpl(this, config);
        });
    }

    /**
     * Return a {@code Path} object by converting the given {@link URI}. The resulting {@code Path} is associated with a {@link FileSystem} that already exists or is constructed automatically.
     *
     * <p> The exact form of the URI is file system provider dependent. In the
     * case of the default provider, the URI scheme is {@code "file"} and the given URI has a non-empty path component, and undefined query, and fragment components. The resulting {@code Path} is associated with the default {@link FileSystems#getDefault default} {@code FileSystem}.
     *
     * <p> If a security manager is installed then a provider implementation
     * may require to check a permission. In the case of the {@link FileSystems#getDefault default} file system, no permission check is required.
     *
    ...
    @Override
    public S3PathImpl getPath(URI uri) throws IllegalArgumentException, FileSystemNotFoundException, SecurityException {
        Objects.requireNonNull(uri);
        return getFileSystem(uri).getPath(uri.getScheme() + ":/" + uri.getPath());
    }

    /**
     * Opens or creates a file, returning a seekable byte channel to access the file. This method works in exactly the manner specified by the {@link Files#newByteChannel(Path, Set, FileAttribute[])} method.
     *
     ...
     * @throws FileAlreadyExistsException if a file of that name already exists and the {@link StandardOpenOption#CREATE_NEW CREATE_NEW} option is specified
     * <i>(optional specific exception)</i>
     * @throws IOException if an I/O error occurs
     * @throws SecurityException In the case of the default provider, and a security manager is installed, the {@link SecurityManager#checkRead(String) checkRead} method is invoked to check read access to the path if the file is opened for reading. The {@link SecurityManager#checkWrite(String) checkWrite} method is invoked to check write access to the path if the file is opened for writing. The {@link SecurityManager#checkDelete(String) checkDelete} method is invoked to check delete access if the file is opened with the {@code DELETE_ON_CLOSE} option.
     */
    @Override
     ...
     * <br>
     * Our implementation will also copy the content of the directory via batched copy operations. This is a variance from some other implementations such as `UnixFileSystemProvider` where directory contents are not copied and the use of the {@code walkFileTree} is suggested to perform deep copies. In S3 this could result in an explosion of API calls which would be both expensive in time and possibly money.
     *
     ...
     * {@link Files#isWritable isWritable} and {@link Files#isExecutable isExecutable} methods to check the accessibility of a file.
     *
     * <p> This method checks the existence of a file and that this Java virtual
     * machine has appropriate privileges that would allow it to access the file according to all the access modes specified in the {@code modes} parameter as follows:
     * <br>
     * <table class="striped">
     ...
     * @throws UnsupportedOperationException an implementation is required to support checking for {@code READ}, {@code WRITE}, and {@code EXECUTE} access. This exception is specified to allow for the {@code Access} enum to be extended in future releases.
    ...
    private CompletableFuture<? extends S3Response> getCompletableFutureForHead(S3PathImpl s3Path) {
        final var fs = s3Path.getFileSystem();
     ...
     * @param path the path to the file
     * @param type the {@code Class} object corresponding to the file attribute view. Must be {@code BasicFileAttributeView.class}
    ...
    /**
     * Reads a file's attributes as a bulk operation. This method works in exactly the manner specified by the {@link Files#readAttributes(Path, Class, LinkOption[])} method.
     *
     * @param path the path to the file
     * @param type the {@code Class} of the file attributes required to read. Supported types are {@code BasicFileAttributes}
    ...
    /**
     * Reads a set of file attributes as a bulk operation. Largely equivalent to {@code readAttributes(Path path, Class<A> type, LinkOption... options)} where the returned object is a map of method names (attributes) to values, filtered on the comma separated {@code attributes}.
     *
     * @param path the path to the file
     * @param attributes the comma separated attributes to read. May be prefixed with "s3:"
     * @param options ignored, S3 has no links
     * @return a map of the attributes returned; may be empty. The map's keys are the attribute names, its values are the attribute values. Returns an empty map if {@code attributes} is empty, or if {@code path} is inferred to be a directory.
     ...
     * @param options options specifying how the file is opened
     * @param attrs an optional list of file attributes to set atomically when creating the file. Currently, ignored.
     * @return a new {@code FileChannel} object representing the specified file
     * @throws IOException if an I/O error occurs
     */
    @Override
    public S3FileChannel newFileChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs)
            throws IOException {

        var fs = getFileSystem(path.toUri());
        var s3SeekableByteChannel = new S3SeekableByteChannel((S3PathImpl) path, fs.client(), options);
    ...
    public AsynchronousFileChannel newAsynchronousFileChannel(Path path,
        ...
        var fs = getFileSystem(path.toUri());
        var s3Client = fs.client();
        var byteChannel = new S3SeekableByteChannel((S3PathImpl) path, s3Client, options);
    ...
    boolean exists(S3AsyncClient s3Client, S3PathImpl path) throws InterruptedException, TimeoutException {
        try {
    ...
    /**
     * This method parses the provided URI into elements useful to address and configure the access to a bucket. These are:
     * <br>
     * - key: the file system key that can be used to uniquely identify a S3 file systems instance (for example for caching purposes) - bucket: the name of the bucked to be addressed - endpoint: non default endpoint where the bucket is located
     * <br>
     * The default implementation in {@code S3FileSystemProvider} treats {@code uri} strictly a AWS S3 URI (see Accessing a bucket using S3:// section <a href="in ">* https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-bucket-</a>intro.html). As such, it returns an empty endpoint and the name of the bucket as key.
     * <br>
     * Subclasses can override this method to implement alternative parsing of the provided URI so that they can implement alternative URI schemes.
     *
     * @param uri the uri to address the bucket
     * @return the information extracted from {@code uri}
     */
    abstract S3FileSystemInfo fileSystemInfo(URI uri);
    ...
    private CompletableFuture<CompletedCopy> copyKey(
            ...
            String sourcePrefix,
            String sourceBucket,
            S3PathImpl targetPath,
            S3TransferManager transferManager,
            Function<S3PathImpl, Boolean> fileExistsAndCannotReplaceFn
    ...
    private Function<S3PathImpl, Boolean> cannotReplaceAndFileExistsCheck(CopyOption[] options, S3AsyncClient s3Client) {
        final var canReplaceFile = Arrays.asList(options).contains(StandardCopyOption.REPLACE_EXISTING);

        return (S3PathImpl destination) -> {
            if (canReplaceFile) {
                return false;
            }
            try {
    ...
    static S3PathImpl checkPath(Path obj) {
        Objects.requireNonNull(obj);
        if (!(obj instanceof S3PathImpl)) {
            throw new ProviderMismatchException();
        }
        return (S3PathImpl) obj;
    }
}
