...
import static software.amazon.nio.spi.s3.S3FileSystemProviderImpl.checkPath;
...
final class S3PathImpl implements S3Path {

    private final S3FileSystemImpl fileSystem;
    private final PosixLikePathRepresentation pathRepresentation;

    private S3PathImpl(S3FileSystemImpl fileSystem, PosixLikePathRepresentation pathRepresentation) {
     ...
     */
    private S3PathImpl from(String path) {
     ...
     */
    static S3PathImpl getPath(S3FileSystemImpl fsForBucket, String first, String... more) {
        if (fsForBucket == null) {
        ...
        return new S3PathImpl(fsForBucket, PosixLikePathRepresentation.of(first, more));
    }
     ...
     */
    @Override
    public S3FileSystemImpl getFileSystem() {
        return fileSystem;
    }
     ...
     */
    @Override
    public S3PathImpl getRoot() {
        return isAbsolute() ? new S3PathImpl(fileSystem, PosixLikePathRepresentation.ROOT) : null;
    }
     ...
     */
    @Override
    public S3PathImpl getName(int index) {
        final var elements = pathRepresentation.elements();
     ...
     */
    @Override
    public S3PathImpl subpath(int beginIndex, int endIndex) {
    ...
    public S3PathImpl normalize() {
        ...
        return S3PathImpl.getPath(fileSystem, String.join(PATH_SEPARATOR, realElements));
    }
     ...
     */
    @Override
    public S3PathImpl resolve(Path other) {
     ...
     */
    @Override
    public S3PathImpl resolve(String other) {
     ...
     */
    @Override
    public S3PathImpl resolveSibling(Path other) {
     ...
     */
    @Override
    public S3PathImpl resolveSibling(String other) {
        return getParent().resolve(other);
    }
     ...
     */
    @Override
    public S3PathImpl relativize(Path other) {
        ...
        return new S3PathImpl(getFileSystem(), new PosixLikePathRepresentation(relativePath));
    }

    private S3PathImpl getRelativePathFromDifference(S3PathImpl otherPath, int otherNameCount, int differenceCount, int parentDirCount) {
     ...
     */
    @Override
    public S3PathImpl toAbsolutePath() {
        if (isAbsolute()) {
            return this;
        }

        return new S3PathImpl(fileSystem, PosixLikePathRepresentation.of(PATH_SEPARATOR, pathRepresentation.toString()));
    }
     ...
     */
    @Override
    public S3PathImpl toRealPath(LinkOption... options) {
        var p = this;
        if (!isAbsolute()) {
            p = toAbsolutePath();
        }

        return S3PathImpl.getPath(fileSystem, PATH_SEPARATOR, p.normalize().toString());
    }
    ...
    public boolean equals(Object other) {
        ...
        return other instanceof S3PathImpl
            && Objects.equals(((S3PathImpl) other).bucketName(), this.bucketName())
            && Objects.equals(((S3PathImpl) other).toRealPath(NOFOLLOW_LINKS).pathRepresentation,
